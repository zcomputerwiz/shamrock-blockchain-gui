...
import { AlertDialog, Card, Flex } from '@shamrock/core';
import isElectron from 'is-electron';
import { newBuy, newSell, addTrade, resetTrades } from '../../modules/trade';
import { shamrock_to_clover, colouredcoin_to_clover } from '../../util/shamrock';
  ...
  function handleAdd() {
    ...
    const clover =
      wallets[wallet_id.value].type === COLOURED_COIN
        ? colouredcoin_to_clover(amount_input.value)
        : shamrock_to_clover(amount_input.value);

    const trade =
      buy_or_sell.value === 1
        ? newBuy(clover, wallet_id.value)
        : newSell(clover, wallet_id.value);
