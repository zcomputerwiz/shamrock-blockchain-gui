...
import shamrockEnvironment from '../util/shamrockEnvironment';
import shamrockConfig from '../util/config';
...
if (!handleSquirrelEvent()) {
  ...
  const ensureCorrectEnvironment = () => {
    ...
    if (!shamrockEnvironment.guessPackaged() && !('VIRTUAL_ENV' in process.env)) {
  ...
  if (ensureSingleInstance() && ensureCorrectEnvironment()) {
    ...
    shamrockConfig.loadConfig('mainnet');
    ...
    const createWindow = async () => {
      ...
      if (!shamrockConfig.manageDaemonLifetime()) {
      ...
      mainWindow.on('close', (e) => {
        ...
        if (decidedToClose || !shamrockConfig.manageDaemonLifetime()) {
          return;
        }
        e.preventDefault();
        if (!isClosing) {
    ...
    const appReady = async () => {
      ...
      // if the daemon isn't local we aren't going to try to start/stop it
      if (shamrockConfig.manageDaemonLifetime()) {
        shamrockEnvironment.startShamrockDaemon();
      }
    };
  ...
  const getMenuTemplate = () => {
    const template = [
      ...
      },
      {
        ...
        submenu: [
          {
            label: i18n._(/* i18n */ { id: 'Shamrock Blockchain Wiki' }),
            click: () => {
              openExternal(
                'https://github.com/Shamrock-Network/shamrock-blockchain/wiki',
              );
            },
          },
          {
            ...
            click: () => {
              openExternal(
                'https://github.com/Shamrock-Network/shamrock-blockchain/wiki/FAQ',
              );
            },
          },
          {
            ...
            click: () => {
              openExternal(
                'https://github.com/Shamrock-Network/shamrock-blockchain/releases',
              );
            },
          },
          {
            ...
            click: () => {
              openExternal(
                'https://github.com/Shamrock-Network/shamrock-blockchain/blob/master/CONTRIBUTING.md',
              );
            },
          },
          {
          ...
          },
          {
            ...
            click: () => {
              openExternal(
                'https://github.com/Shamrock-Network/shamrock-blockchain/issues',
              );
            },
          },
          {
            ...
            click: () => {
              openExternal('https://keybase.io/team/shamrock_network.public');
            },
          },
          {
            label: i18n._(/* i18n */ { id: 'Follow on Twitter' }),
            click: () => {
              openExternal('https://twitter.com/shamrock_project');
            },
          },
        ],
      },
    ...
    if (process.platform === 'darwin') {
      // Shamrock Blockchain menu (Mac)
      template.unshift({
        label: i18n._(/* i18n */ { id: 'shamrock' }),
        submenu: [
          {
            label: i18n._(/* i18n */ { id: 'About Shamrock Blockchain' }),
            click: () => {
              openAbout();
            },
          },
