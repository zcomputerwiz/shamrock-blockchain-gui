...
const PY_FOLDER = '../shamrock/daemon';
...
const getShamrockVersion = () => {
  let version = null;
  const exePath = getExecutablePath('shamrock');
  // first see if we can get a shamrock exe in a standard location relative to where we are
  try {
    ...
    // that didn't work, let's try as if we're in the venv or shamrock is on the path
    try {
      version = child_process
